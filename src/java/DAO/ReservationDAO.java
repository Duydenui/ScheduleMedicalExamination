...
public class ReservationDAO {
    ...
    public ChildrenInfo getChildrenInfoByReservationDetailId(int reservationDetailId) {
    try {
        Connection conn = new DBContext().getConnection();
        ...
        PreparedStatement ps = conn.prepareStatement(sql);
        ps.setInt(1, reservationDetailId);
        ResultSet rs = ps.executeQuery();
        
        if (rs.next()) {
            String childrenName = rs.getString("children_name");
            String childrenGender = rs.getString("children_gender");
            int childrenAge = rs.getInt("children_age");

            ChildrenInfo childrenInfo = new ChildrenInfo(childrenName, childrenGender, childrenAge);
    ...
    public List<reservationlist> getAllReservationList2() {
        ...
        try {
            Connection conn = new DBContext().getConnection();
            String sql = "select r.reservation_id, r.created_date, u.user_fullname ,s.service_title, d.num_of_person , cl.children_name, r.total_price, r.reservation_status from reservation r join [user] u\n"
            ...
            while (rs.next()) {
                reservationlist rev = new reservationlist(rs.getInt("reservation_id"), rs.getDate("created_date"), rs.getString("user_fullname"),
                        rs.getString("service_title"), rs.getInt("num_of_person"), rs.getString("children_name"), rs.getFloat("total_price"), rs.getInt("reservation_status"));
//rs.getInt("reservation_id"),rs.getDate("created_date"),rs.getString("user_fullname") ,rs.getString("user_fullname")
    ...
    public reservationmail getReservationDetailByReservationID(int id) {
        try {
            Connection conn = new DBContext().getConnection();
            String sql = "SELECT r.reservation_id, r.service_id, r.price, r.doctor_id, r.nurse_id, r.slot, u.[user_id], u.user_fullname, s.service_title\n"
                    + "FROM dbo.reservation_detail r\n"
                    + "INNER JOIN dbo.[user] u ON r.doctor_id = u.[user_id]\n"
                    + "INNER JOIN dbo.service s ON r.service_id = s.service_id\n"
                    + "where reservation_id = ?";
            PreparedStatement ps = conn.prepareStatement(sql);
            ps.setInt(1, id);
            ResultSet rs = ps.executeQuery();
            ...
            while (rs.next()) {
//                return new reservation(rs.getInt("reservation_id"), rs.getInt("user_id"), rs.getFloat("total_price"), rs.getString("note"), rs.getInt("reservation_status"), rs.getDate("created_date"));
                  return new reservationmail(rs.getInt("reservation_id"),
        ...
        System.out.println(re.getReservationDetailByReservationID(67));
    }
}
