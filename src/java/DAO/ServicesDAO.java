...
public class ServicesDAO extends DBContext {
    ...
    public service getServiceById(int id) {
        try {
            Connection conn = new DBContext().getConnection();
            String sql = "Select sv.service_id, sv.service_title, sv.service_bi, sv.service_created_date, sv.category_id, sv.service_price, sv.service_discount, sv.service_detail, sv.service_rateStar, sv.service_vote, si.image_link from service sv \n"
                    + "  join service_image si \n"
                    + "  on sv.service_id = si.image_id";
            PreparedStatement ps = conn.prepareStatement(sql);
            ps.setInt(1, id);
            ResultSet rs = ps.executeQuery();
            while (rs.next()) {
...
// insert service_image
    ...
    public List<servicelist> getPaging(int index) {
        String query = "select s.service_id,si.image_link, c.category_name, s.service_title, s.service_bi, s.service_price, s.service_discount, st.service_status \n"
                + "from category as c inner join service as s on c.category_id = s.category_id\n"
                + "inner join service_image as si on s.service_id = si.service_id\n"
                + "inner join service_status st on s.service_id = st.service_id\n"
                + "order by s.service_created_date offset ? rows fetch first 6 rows only";
        ...
        try {
            ...
            rs = ps.executeQuery();
            while (rs.next()) {
                list.add(new servicelist(rs.getInt(1),
                        rs.getString(2),
                        rs.getString(3),
                        rs.getString(4),
                        rs.getString(5),
    ...
    public List<servicelist> getServiceByCategory(String cid) {
        List<servicelist> list = new ArrayList<>();
        String query = "SELECT s.service_id, si.image_link, c.category_name, s.service_title, s.service_bi, s.service_price, s.service_discount, st.service_status\n"
                + "FROM category AS c INNER JOIN service AS s ON c.category_id = s.category_id\n"
                + "INNER JOIN service_image AS si ON s.service_id = si.service_id\n"
                + "INNER JOIN service_status AS st ON s.service_id = st.service_id\n"
                + "WHERE c.category_id = ?;";
        try {
            conn = new DBContext().getConnection();
            ps = conn.prepareStatement(query);
    ...
    public List<servicelist> serviceSearch(String txtSearch) {
        List<servicelist> list = new ArrayList<>();
        String query = "SELECT s.service_id, si.image_link, c.category_name, s.service_title, s.service_bi, s.service_price, s.service_discount, st.service_status\n"
                + "FROM category AS c INNER JOIN service AS s ON c.category_id = s.category_id\n"
                + "INNER JOIN service_image AS si ON s.service_id = si.service_id\n"
