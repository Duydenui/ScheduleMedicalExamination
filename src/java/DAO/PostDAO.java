...
public class PostDAO {
    ...
    public void createNewBlog(int userId, String title, String bi, String categoryId, String detail, String blogImage, boolean status) {
    try {
        Connection conn = new DBContext().getConnection();
        String sql = "INSERT INTO blogs (user_id, title, bi, category_id, detail, blog_image, view_able) VALUES (?, ?, ?, ?, ?, ?, ?)";
        PreparedStatement ps = conn.prepareStatement(sql);
        ps.setInt(1, userId);
        ...
        ps.setString(6, blogImage);
        ps.setBoolean(7, status);
        ps.executeUpdate();
        // Close the PreparedStatement and Connection here
    } catch (Exception e) {
